...
def _ist_str(fmt: str = "%Y-%m-%d %H:%M:%S IST") -> str:
...
def export_json(results, model: str, dataset_name: str, timestamp: str, pass_rate: float, hall_rate: float, avg_sim: float, avg_lat: float, username: str = ""):
    ...
    report_data = {
        "metadata": {
            ...
            "model": model,
            "user": username or "—",
            "generated_at": _ist_str("%Y-%m-%dT%H:%M:%S IST"),
...
def export_pdf(results, model: str, dataset_name: str, timestamp: str, pass_rate: float, hall_rate: float, avg_sim: float, avg_lat: float, total_results: int, username: str = ""):
    ...
    # Build per-row style commands — failed rows get red bg + white text
    row_styles = [
        # Header row
    ...
    for row_idx, r in enumerate(results[:50], 1):
        if not r.get("passed"):
            row_styles.append(("BACKGROUND", (0, row_idx), (-1, row_idx), colors.HexColor("#ef4444")))
    ...
    user_line = f"  \u2022  User: {username}" if username else ""
    content.append(Paragraph(
        f"Report generated by TrustLLM  \u2022  {_ist_str()}  \u2022  Model: {model or '\u2014'}{user_line}",
        footer_style,
    ))
