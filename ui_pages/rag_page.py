...
def _tab_chat():
    ...
    st.caption("Ask questions about your uploaded documents. Answers are generated via Groq cloud API.")

    cc1, cc2 = st.columns([2, 1])
    with cc1:
        model = st.selectbox("Model", AVAILABLE_MODELS, key="rag_model",
                             help="llama-3.1-8b-instant is fastest. llama-3.3-70b-versatile is most capable.")
    with cc2:
...
def render():
    ...
    st.caption("Build a vector knowledge base, query it with a cloud LLM (Groq), and evaluate response quality.")
